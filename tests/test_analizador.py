...
class TestAnalizador(unittest.TestCase):
    ...
    def test_compras_por_sector_valores_no_negativos(self):
        compras = self.analizador.compras_por_sector()
    self.assertTrue(all(float(v) >= 0 for v in compras.values()))

    def test_sector_con_mayor_compras(self):
        sector = self.analizador.sector_con_mayor_compras()
        self.assertIsInstance(sector, str)
        self.assertGreater(len(sector), 1)
